import { ComingSoon } from '@/components/sections/home/coming-soon'
import { SectionHeading, SectionRail } from '@/components/sections/home/section-heading'
import { EVENT_YEAR } from '@/content/constants'

// Restore this import with the embedded agenda block once the schedule is confirmed.
// import { LINKS } from '@/content/constants'

export function Agenda() {
  return (
    <section id="agenda" className="mt-28 scroll-mt-28">
      <SectionRail>
        <SectionHeading>Agenda</SectionHeading>

        {/*
          The agenda is temporarily hidden while this year's schedule is unconfirmed.
          Restore the LINKS import above and this block, then comment out ComingSoon.

          <div className="relative mb-4 mt-6 w-full overflow-hidden rounded-xl border border-primary/20 bg-card/40 p-1.5 shadow-[0_24px_70px_-36px_rgba(0,98,155,0.75)] dark:bg-[#020611]">
            <iframe
              title="IEEE SPAC event agenda"
              className="h-[500px] w-full rounded-lg border-none bg-white transition-[filter] duration-300 dark:invert dark:hue-rotate-180 sm:h-[600px] md:h-[700px]"
              src={LINKS.AGENDA}
              loading="lazy"
              sandbox="allow-scripts allow-same-origin allow-forms allow-popups"
              allowFullScreen
            />
          </div>
          <p className="text-right text-sm text-muted-foreground">
            Having trouble with the embedded schedule?
            {' '}
            <a className="font-medium text-primary underline underline-offset-4" href={LINKS.AGENDA} target="_blank" rel="noopener noreferrer">
              Open the agenda in a new tab
            </a>
            .
          </p>
        */}

        <ComingSoon
          action={{ href: '#contact', text: 'Ask about the agenda' }}
          description={(
            <>
              The SPAC
              {' '}
              {EVENT_YEAR}
              {' '}
              agenda will be published here once the schedule is finalized.
            </>
          )}
        />
      </SectionRail>
    </section>
  )
}
